...
const HeroSection = () => {
  ...
  return (
    <main className="dark:bg-black bg-black py-10 md:py-20">
        ...
        <div className="container mx-auto px-6 flex relative py-16 lg:py-40 ">
          <div className="sm:w-2/3 lg:w-2/5 flex flex-col relative z-20 lg:ml-40">
            <h1 className="font-bebas-neue uppercase text-6xl sm:text-8xl font-black flex flex-col leading-none dark:text-white text-gray-800  ">
              MORGANA
              <span className="text-4xl sm:text-4xl text-blue-900">VALUE FOR THE FUTURE</span>
            </h1>
          ...
          <div className="absolute top-0 left-0 w-full h-full flex justify-center items-center pointer-events-none">
            {[...Array(20)].map((_, index) => (
              <div
                key={index}
                className={`absolute w-4 h-4 md:w-6 md:h-6 rounded-full ${index % 2 === 0 ? 'bg-white' : 'bg-yellow-400'} animate-float`}
                style={{
